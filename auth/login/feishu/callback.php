...
if (empty($_SESSION['social_login_provider']) || $_SESSION['social_login_provider'] !== 'feishu') {
...
$callbackUrl = rtrim($baseUrl, '/') . '/auth/login/feishu/callback.php';
...
$stmt = $pdo->prepare("SELECT user_id FROM user_connections WHERE provider = 'feishu' AND provider_user_id = ? LIMIT 1");
...
auditLog('auth.login.success', $userId, null, ['provider' => 'feishu'], 'success');
