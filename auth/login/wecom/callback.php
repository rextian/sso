...
if (empty($_SESSION['social_login_provider']) || $_SESSION['social_login_provider'] !== 'wecom' || !hash_equals($_SESSION['social_login_state'] ?? '', $state) || (time() - ($_SESSION['social_login_at'] ?? 0)) > 600) {
    unset($_SESSION['social_login_provider'], $_SESSION['social_login_state'], $_SESSION['social_login_at'], $_SESSION['social_login_redirect']);
...
$tokenUrl = 'https://qyapi.weixin.qq.com/cgi-bin/gettoken?' . http_build_query(['corpid' => $corpId, 'corpsecret' => $secret]);
...
$userInfoUrl = 'https://qyapi.weixin.qq.com/cgi-bin/auth/getuserinfo?' . http_build_query(['access_token' => $accessToken, 'code' => $code]);
...
$stmt = $pdo->prepare("SELECT user_id FROM user_connections WHERE provider = 'wecom' AND provider_user_id = ? LIMIT 1");
...
auditLog('auth.login.success', $userId, null, ['provider' => 'wecom'], 'success');
